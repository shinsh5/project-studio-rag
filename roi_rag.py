...
def _stb_automerge_context(
    ...
    if selected_parents:
        parent_texts = "\n\n".join([
            f"[Parent Chunk #{pid}]\n{trim(parent_chunks[pid])}"
            for pid in selected_parents
        ])
        body = f"Extended Context (Small-to-Big):\n{parent_texts}"
    elif candidates:
        body = "Extended Context (Small-to-Big): parent text already included above."
    else:
...
def get_roi_rag_pipeline():
    ...
    def run_pipeline(
        ...
        for idx, score in zip(eu_indices, similarity_scores):
            ...
            if use_stb_path:
                if active_stb_mode == "all_segments":
                    body = _stb_all_segments_context(eu, segments, _trim)
                else:
        ...
        usage: dict = {}
        cache_hit = False
        # BM25 선별과 검색 모드 모두 프롬프트를 바꾸므로 캐시 정체성에 포함한다.
        bm25_tag = (
        ...
        index_version = (
            ...
            f":{retrieval_signature}:{bm25_tag}"
        )
        ...
        return {
            ...
            "pipeline_metrics": {
                ...
                "small_to_big_enabled": bool(use_stb_path),
